...
void	fill_player(t_game *game, float x, float y, int color, int cell_size)
{
	int	center_size;
	int	center_start;
	int	dx;
	...
	while (dy < center_size)
	{
		while (dx < center_size)
		{
			my_mlx_pixel_put(&game->mlx,
				...
				(y * cell_size) + center_start + dy, color);
			dx++;
		}
		dx = 0;
		dy++;
...
void	fill_pixels2(t_game *game, float x, float y, int color)
{
	...
	fill_player(game, x, y, 0x000000, cell_size);
}

void	draw_player(t_game *game)
{
	fill_pixels2(game, game->player.pos_y - 0.5f, game->player.pos_x - 0.5f, 0xFF0000);
}
